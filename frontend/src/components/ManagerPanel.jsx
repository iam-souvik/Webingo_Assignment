...
const ManangerPanel = () => {
     ...
     useEffect(() => {
          if (JSON.parse(localStorage.getItem("USER"))?.role !== "manager") {
               Toast({
                    description: "You're not a manager, so you aren't authorized to access this page!",
                    status: 'warning',
                    duration: 4000,
                    isClosable: true,
                    position: "top"
     ...
     return (
          <div>
               {loading ? (
                    ...
                    <>
                         <Center pt="20px">
                              <Link to="/createuser"><Button colorScheme='yellow' >Create User</Button></Link>
                         </Center>

                         <Heading textAlign="center" margin={"auto"} mt={"50px"}>Manager Page</Heading>
                         <TableContainer>
